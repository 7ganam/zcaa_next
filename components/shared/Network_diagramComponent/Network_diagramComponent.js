...
const draw_glope = (screen_width) => {
  ...
  globe.loadPlugin(
    window.planetaryjs.plugins.earth({
  ...
  if (screen_width > 500) {
    // allow drag only on big screens
    globe.loadPlugin(
      window.planetaryjs.plugins.drag({
  ...
  useEffect(() => {
    ...
    if (x > 500) {
      const jp = $("#network_div").jParticle({
        particlesNumber: particles_number,
        background: "CCE6EB",
        color: "white",
        createLinkDist: 120,
        speed: 66,
      ...
      <div id="network_card" style={{}}>
        <div id="network_card_div">
          <div id="network_card_text">
            connecting ZC Alumni around the world
          </div>
        </div>
      </div>
    </div>
