  ...
  createParticlesSandbox = function createParticlesSandbox(element, params) {
    ...
    ParticlesSandbox.initParams = function initParams(params) {
      if (
        params &&
        params.color &&
        (!params.particle || (params.particle && !params.particle.color))
      ) {
        if (!params.particle) {
          params.particle = {};
        }
    ...
    ParticlesSandbox.initHTML = function initHTML(element) {
      ...
      canvas.element.get(0).width = ParticlesSandbox.params.width
        ? ParticlesSandbox.params.width
        : canvas.container.width();
      canvas.element.get(0).height = ParticlesSandbox.params.height
        ? ParticlesSandbox.params.height
    ...
    ParticlesSandbox.initParticles = function initParticles() {
      ...
      for (; i < count; i += 1) {
        ParticlesSandbox.particles.push(
          createParticle(
    ...
    ParticlesSandbox.initEvents = function initEvents() {
      ...
      ParticlesSandbox.canvas.element.mousemove(function mouseMoveCallback(e) {
        ParticlesSandbox.mouse = $.extend(
          ParticlesSandbox.mouse,
          Utils.getMousePosition(e, ParticlesSandbox.canvas.element[0])
        );
      });
    };
    ...
    ParticlesSandbox.draw = function draw() {
      ...
      canvas.context.clearRect(
        0,
        0,
        canvas.element.get(0).width,
        canvas.element.get(0).height
      );
    ...
    ParticlesSandbox.drawLink = function drawLink(x, y, x2, y2) {
      ...
      if (
        Utils.getDistance(x, y, x2, y2) <=
        ParticlesSandbox.params.createLinkDist
      ) {
    ...
    ParticlesSandbox.getOpacityLink = function getOpacityLink(x, y, x2, y2) {
      ...
      if (dist <= linkDist) {
        ...
        opacity =
          1 - ((dist - linkDist) * 100) / (createLinkDist - linkDist) / 100;
      }
    ...
    createParticle = function createParticle(canvas, params) {
      ...
      Particle.initPosition = function initPosition() {
        Particle.x = Utils.getRandNumber(
          0 + Particle.radius,
          Particle.canvas.element.width - Particle.radius
        );
        Particle.y = Utils.getRandNumber(
          0 + Particle.radius,
          Particle.canvas.element.height - Particle.radius
        );
      };
      ...
      Particle.initSize = function initSize() {
        Particle.size = Utils.getRandNumber(
          Particle.params.minSize,
          Particle.params.maxSize
        );
      ...
      Particle.initVectors = function initVectors() {
        do {
          Particle.vector.x = Utils.getRandNumber(
            -Particle.params.speed / 60,
            Particle.params.speed / 60,
            false
          );
          Particle.vector.y = Utils.getRandNumber(
            -Particle.params.speed / 60,
            Particle.params.speed / 60,
            false
          );
      ...
      Particle.update = function update() {
        ...
        if (
          0 > Particle.x - Particle.radius ||
          Particle.x + Particle.radius > Particle.canvas.element.width
        ) {
          Particle.vector.x = -Particle.vector.x;
        }

        if (
          0 > Particle.y - Particle.radius ||
          Particle.y + Particle.radius > Particle.canvas.element.height
        ) {
