...
const register_experience_fields = async (experice_fields) => {
    ...
    const registered_exp_fields = experice_fields.map(exp_field => fetch_field_or_create_new_one(exp_field.value, exp_field.label))
...
const fetch_field_or_create_new_one = async (field_id, field_label) => { // dont catch errors here .. errors should be handled in upper comps
    ...
    if (ObjectId.isValid(field_id)) { // only look in the data base if the field_id is a valid mongodb id
    ...
    if (found_exp_field == undefined || found_exp_field.length < 1) {// in some cases the field is registered but the front end fails to fetch data and sends again ..this causes a duplicate field ... this is to double check based on the value itself 
...
const create_exp_field = async (field_label) => {  // dont catch errors here .. errors should be handled in upper comps
    await dbConnect()
...
export { create_exp_field, fetch_field_or_create_new_one, register_experience_fields, fetch_all_experience_fields }