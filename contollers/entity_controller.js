...
const register_entities = async (entities) => {
    for (entity of entities) {
        let fetched_entity = await fetch_entity_or_create_new_one(entity.entity_name.value, entity.entity_name.label)
...
const fetch_entity_or_create_new_one = async (entity_id, entity_label) => { // dont catch errors here .. errors should be handled in upper comps
    ...
    if (ObjectId.isValid(entity_id)) { // only look in the data base if the entity_id is a valid mongodb id
    ...
    if (found_entity == undefined || found_entity.length < 1) {// in some cases the entity is registered but the front end fails to fetch data and sends again ..this causes a duplicate entity ... this is to double check based on the value itself 
...
const create_entity = async (entity_label) => {  // dont catch errors here .. errors should be handled in upper comps
    await dbConnect()
